...
- AGENT-8: The agent may await user input or finish; it cannot select a terminal blocked state. When it needs help, it asks through Telegram and keeps the conversation resumable. A reply to a plain help message resumes the agent when no proposal responses remain required. The app owns technical failure handling and preserves recovery state when execution fails.
...
- AGENT-11: After critic feedback is applied, one read-only consolidator call checks each candidate against current opinions and provenance. Its root result contains a required non-empty reasoning string followed by one tagged decision object: `independent` keeps the complete candidate new; `attach` moves a non-empty evidence subset to one existing opinion without changing its text; and `revise` moves a non-empty evidence subset and replaces that opinion's text with the complete non-empty revision.
- AGENT-12: Attach and revise operations can move all or part of the candidate evidence. Moved evidence belongs only to the named existing-opinion proposal, and evidence not moved belongs only to the residual add proposal. After a partial move, the main agent authors that residual proposal around the remaining evidence without a second critic call or a candidate-file edit.
...
- PROPOSAL-10: Existing opinions do not suppress extraction of independent candidates. An `independent` decision produces an add proposal. A full evidence move produces only an attach-evidence or revise-existing proposal. A partial evidence move also produces a rewritten add proposal supported by the remaining evidence. Routing changes only proposal text and conversation state; the frozen candidate snapshot stays unchanged.
- PROPOSAL-11: The main agent authors all Telegram proposal text. Consolidation controls the initial operation; later user feedback may change an add, attach, or revise proposal. The agent preserves selected-evidence ownership and the frozen candidate snapshot, then requests approval of the revised proposal before applying it. An attach-evidence proposal names the target opinion ID and shows its existing text unchanged under “Opinion”; approval adds evidence without changing that text. No deterministic renderer applies consolidation results.
...
- TELEGRAM-3: Only `TELEGRAM_ALLOWED_CHAT_ID` may drive the run through callbacks, replies, or standalone text.
...
- TELEGRAM-11: Standalone text from the allowed chat resumes an awaiting conversation immediately, with any recorded current-turn responses. It is feedback, not approval. It does not start or reopen runs that are running, completed, or stopped.
