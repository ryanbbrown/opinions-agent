...
- Set `RAILPACK_DEPLOY_APT_PACKAGES="git ripgrep"` on the web service. Git clones and pushes the opinions
  repository; the agent's document-search tool requires the `rg` executable from ripgrep.
...
`eval/opinion_targets.jsonl` is the checked-in ground truth: per eval week it lists canonical target opinions, required evidence, and selected evidence that should not become opinions. `eval/evidence_availability.jsonl` can delay evidence to the week when it was fully processed; evals apply those timestamps only to disposable corpus copies and leave `.readwise` unchanged. `cproxy run --port 8113 --chains-max 500 -- uv run opinions-agent eval run --weeks W04 ... W13` runs the historical v1 proposal eval. `opinions-agent eval v2 run` runs the same independent weekly cases and adds pre-consolidation candidate quality and grouping, deterministic operation accuracy, and operation-gated final quality. `candidate_quality` grades all evidence-linked candidates as a set, while `candidate_independent_quality` requires at least one candidate to cover the complete add target by itself. `candidate_grouping` separately compares the frozen candidate evidence partition with the target partition and reports over-merged and under-merged evidence pairs. A v2 target receives operation credit only when all its evidence uses the canonical route: add a new opinion, or revise the target's canonical base opinion. The current reviewed targets contain no evidence-only attach operation, so an attach decision counts as the wrong operation. `eval v2 rescore --from-experiment <name>` applies v2 to stored Braintrust output without rerunning the opinion agent; candidate metrics are available when the stored output includes its candidate snapshot. V1 and v2 sync to separate Braintrust datasets. Both use fake Telegram, stop before approvals, tag experiment rows by week, and attach full agent traces.
