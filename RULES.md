...
- **The concrete anchors.** If the evidence hangs the claim on a named term, law, example, product, number, or formula, keep that name in the opinion sentence itself. This includes illustrative cases: when the argument is built around a named company, product, or situation, the opinion should carry that name and its role in the claim ("...as X's case shows"), not just the abstracted lesson — an example the source reasons through is part of the claim, not decoration. Use the argument's own vocabulary for its key categories and distinctions; do not swap named specifics or the source's own terms for generic paraphrases.
